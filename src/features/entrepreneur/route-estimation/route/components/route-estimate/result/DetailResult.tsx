...
import { StartPoint, EndPoint, Vehicle } from '@/@types/entrepreneur/route-estimation'
...
const DetailResult: React.FC<Props> = (props) => {
  const { data, start_point, end_point } = props
    ...
    <div className='block 2xl:grid grid-cols-2 gap-5 '>
      <div>
        ...
        <section className='mt-3'>
          ...
          <p>{data.towing_axis_weight.join(' : ') || '0'}</p>
        </section>
        <section className='mt-3'>
          <h5>น้ำหนักลงเพลา กึ่งรถพ่วง (กิโลกรัม)</h5>
          <p>{data.semi_trailer_axis_weight.join(' : ') || '0'}</p>
        </section>
        <section className='mt-3'>
          <h5>ต้นทาง</h5>
          <p>{start_point.coordinates.join(' , ') || '-'}</p>
        </section>
        <section className='mt-3'>
          <h5>ปลายทาง</h5>
          <p>{end_point.coordinates.join(' , ') || '-'}</p>
        </section>
      </div>
