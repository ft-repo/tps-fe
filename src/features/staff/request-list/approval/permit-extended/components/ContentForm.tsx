...
const ContentForm: React.FC<Props> = (props) => {
  ...
  const form = useForm<FieldType>({
    defaultValues: {
      ...
      remark: petition_extended_status[2]?.remark || '',
    },
    disabled: disabled
  })
      ...
      if (response.status === 200) {
        Modal.success({
          ...
          onOk: () => {
            ...
            dispatch(getAdminPetitionHistoryData(petition_history_extended.overview.search))
  ...
  }, [state?.petition_id, state?.status_id, dispatch, navigate, petition_extended.overview.search, petition_history_extended.overview.search])
        ...
        setValue('file_id.file', [
          {
            ...
            // name: extractFileName(String(petition_extended_status[2]?.document_url)),
            name: 'เอกสารใบอนุญาต',
            // percent: 100,
            uid: '1',
            status: 'done',
  ...
  useEffect(() => {
    if (petition_extended_status[2]?.document_url) {
      if (extractUrl(petition_extended_status[2]?.document_url)) {
