...
const ContentForm: React.FC<Props> = (props) => {
        ...
        setValue('file_id.file', [
          {
            ...
            // name: extractFileName(String(petition_status[0]?.document_url)),
            name: 'เอกสารตอบกลับ',
            // percent: 100,
            uid: '1',
            status: 'done',
  ...
  useEffect(() => {
    if (petition_status[0]?.document_url) {
      if (extractUrl(petition_status[0]?.document_url)) {
        fetchImage(extractUrl(petition_status[0]?.document_url))
      }
    }
