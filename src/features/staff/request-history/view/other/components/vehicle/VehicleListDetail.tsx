...
import { Col, Descriptions, DescriptionsProps, Image, message, Row, Spin } from 'antd'
...
const VehicleListDetail: React.FC<Props> = (props) => {
  const { item } = props
  const dispatch = useAppDispatch()
  const { loading } = useAppSelector(state => state.layout)
  const [towingUrl, setTowingUrl] = useState<string>('')
  const [semiUrl, setsemiUrl] = useState<string>('')
  ...
  const showFile = useCallback(async (stateType: 'towing' | 'semi' | 'etc', fileUrl: string) => {
  ...
  useEffect(() => {
    if (item?.towing_vehicle?.vehicle_picture?.front_rear_url) {
      showFile('towing', extractUrl(item?.towing_vehicle?.vehicle_picture?.front_rear_url))
    }
    if (item?.semi_trailer_vehicle?.vehicle_picture?.front_rear_url) {
      showFile('semi', extractUrl(item?.semi_trailer_vehicle?.vehicle_picture?.front_rear_url))
    }
    if (item?.etc_vehicle?.vehicle_picture?.front_rear_url) {
      showFile('etc', extractUrl(item?.etc_vehicle?.vehicle_picture?.front_rear_url))
    }
  }, [item, extractUrl, showFile])
...
export default React.memo<Props>(VehicleListDetail)
