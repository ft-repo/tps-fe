...
import { AdminPetitionDetail, VehicleList } from '@/@types/reducer/petition';
...
const PermitForm: React.FC<Props> = (props) => {
  ...
  const convertAddress = useCallback((address: string) => {
    ...
    const pattern = /ตำบล\/แขวง\s+(.+?)\s+อำเภอเขต(.+?)\s+จังหวัด(.+?)\s+(\d{5})/;
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.date_section}>
          <Text>เขียนที่</Text>
          <Text style={styles.underline_long}>{data?.document?.business_name || '-'}</Text>
        </View>
        {/* วันที่ */}
        <View style={styles.date_section}>
          <Text>วันที่</Text>
          <Text style={styles.underline_short}>{dayjs(data?.document?.start_date).format('DD') || '-'}</Text>
          <Text>เดือน</Text>
          <Text style={styles.underline_short}>{dayjs(data?.document?.start_date).format('MMMM') || '-'}</Text>
          <Text>พ.ศ.</Text>
          <Text style={styles.underline_short}>{dayjs(data?.document?.start_date).format('BBBB') || '-'}</Text>
        </View>
        {/* เรียน เรื่อง */}
        ...
        <View style={[styles.section, styles.indent]}>
          ...
          <View style={styles.row}>
            ...
            <Text>ชื่อ</Text>
            <Text style={styles.underline_full}>{data?.document?.business_name || '-'}</Text>
          </View>
        </View>
        ...
        <View style={styles.row}>
          ...
          <Text style={[styles.underline_custom, { minWidth: 170 }]}>{data?.document?.registration_no || '-'}</Text>
        ...
        <View style={styles.row}>
          ...
          <Text style={styles.underline_full}>{'-'}</Text>
          <Text>แขวง/ตำบล</Text>
          <Text style={styles.underline_full}>{convertAddress(data?.document?.address)?.sub_district || '-'}</Text>
        </View>
        <View style={styles.row}>
          <Text>เขต/อำเภอ</Text>
          <Text style={styles.underline_full}>{convertAddress(data?.document?.address)?.district || '-'}</Text>
          <Text>จังหวัด</Text>
          <Text style={styles.underline_full}>{convertAddress(data?.document?.address)?.province || '-'}</Text>
          <Text>โทรศัพท์</Text>
          <Text style={styles.underline_full}>{data?.document?.business_phone_no || '-'}</Text>
        </View>
        <View style={[styles.row, styles.indent]}>
        ...
        <View style={[styles.row, styles.indent]}>
          ...
          <Text>เลขที่ทะเบียน</Text>
          <Text style={styles.underline_full}>{item?.towing_vehicle?.plate_no || <>&nbsp;</>}</Text>
        </View>
        <View style={styles.row}>
          <Text>จังหวัด</Text>
          <Text style={[styles.underline_custom, { minWidth: 60 }]}>{item?.towing_vehicle?.plate_province || <>&nbsp;</>}</Text>
          <Text>สี</Text>
          <Text style={[styles.underline_custom, { minWidth: 50 }]}>&nbsp;</Text>
          <Text>โดยมีจำนวนเพลา</Text>
          <Text style={[styles.underline_custom, { minWidth: 50 }]}>{item?.towing_vehicle?.axis_type?.axis_number || 0}</Text>
          <Text>เพลา</Text>
          <Text> น้ำหนักลงเพลา</Text>
          <Text style={styles.underline_full}>{item?.towing_vehicle.axis_weight[0] || 0} {item?.towing_vehicle.axis_weight[1] || 0}</Text>
        </View>
        <View style={styles.row}>
          <Text style={[styles.underline_custom, { minWidth: 40 }]}>{item?.towing_vehicle.axis_weight[2] || 0}</Text>
          <Text>-</Text>
          <Text style={[styles.underline_custom, { minWidth: 40 }]}>{item?.towing_vehicle.axis_weight[3] || 0}</Text>
          <Text>-</Text>
          <Text style={[styles.underline_custom, { minWidth: 40 }]}>{item?.towing_vehicle.axis_weight[4] || 0}</Text>
          <Text>-</Text>
          <Text style={[styles.underline_custom, { minWidth: 40 }]}>{item?.towing_vehicle.axis_weight[5] || 0}</Text>
          <Text>-</Text>
          <Text style={[styles.underline_custom, { minWidth: 40 }]}>{item?.towing_vehicle.axis_weight[6] || 0}</Text>
          <Text>กิโลกรัม</Text>
          <Text> น้ำหนักรวม</Text>
          <Text style={[styles.underline_custom, { minWidth: 60 }]}>{item?.towing_vehicle?.weight || 0}</Text>
          <Text>กิโลกรัม</Text>
        </View>
        {/* ลักษณะ 2 */}
        <View style={[styles.row, styles.indent]}>
          ...
          <Text>เลขที่ทะเบียน</Text>
          <Text style={styles.underline_full}>{item?.semi_trailer_vehicle?.plate_no || <>&nbsp;</>}</Text>
        </View>
        <View style={styles.row}>
          <Text>จังหวัด</Text>
          <Text style={[styles.underline_custom, { minWidth: 60 }]}>{item?.semi_trailer_vehicle?.plate_province || <>&nbsp;</>}</Text>
          <Text>สี</Text>
          <Text style={[styles.underline_custom, { minWidth: 50 }]}>&nbsp;</Text>
          <Text>โดยมีจำนวนเพลา</Text>
          <Text style={[styles.underline_custom, { minWidth: 50 }]}>{item?.semi_trailer_vehicle?.axis_type?.axis_number || 0}</Text>
          <Text>เพลา</Text>
          <Text> น้ำหนักลงเพลา</Text>
          <Text style={styles.underline_full}>{item?.semi_trailer_vehicle.axis_weight[0] || 0} {item?.semi_trailer_vehicle.axis_weight[1] || 0}</Text>
        </View>
        <View style={styles.row}>
          <Text style={[styles.underline_custom, { minWidth: 40 }]}>{item?.semi_trailer_vehicle.axis_weight[2] || 0}</Text>
          <Text>-</Text>
          <Text style={[styles.underline_custom, { minWidth: 40 }]}>{item?.semi_trailer_vehicle.axis_weight[3] || 0}</Text>
          <Text>-</Text>
          <Text style={[styles.underline_custom, { minWidth: 40 }]}>{item?.semi_trailer_vehicle.axis_weight[4] || 0}</Text>
          <Text>-</Text>
          <Text style={[styles.underline_custom, { minWidth: 40 }]}>{item?.semi_trailer_vehicle.axis_weight[5] || 0}</Text>
          <Text>-</Text>
          <Text style={[styles.underline_custom, { minWidth: 40 }]}>{item?.semi_trailer_vehicle.axis_weight[6] || 0}</Text>
          <Text>กิโลกรัม</Text>
          <Text> น้ำหนักรวม</Text>
          <Text style={[styles.underline_custom, { minWidth: 60 }]}>{item?.semi_trailer_vehicle?.weight || 0}</Text>
          <Text>กิโลกรัม</Text>
        </View>
