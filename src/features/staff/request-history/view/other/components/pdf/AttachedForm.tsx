    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={[styles.header, { textAlign: "center" }]}>
          <Text style={{ fontWeight: "bold" }}>เงื่อนไขประกอบการอนุญาตให้ {data.document.business_name || 'บริษัท มามมุท (ประเทศไทย) จำกัด'} ใช้ยานพาหนะบางชนิด บางประเภทเดินบนทางหลวงชนบท เพื่อขนส่งเครื่องจักรและอุปกรณ์ขนาดใหญ่ จาก {convertToThaiAlp(data.estimate.route.start_point) || 'ท่าเรือแหลมฉบัง จังหวัดชลบุรี'} ไปยัง {convertToThaiAlp(data.estimate.route.end_point) || 'บริษัท ไทยออยล์ จำกัด (มหาชน)'} ผ่านทางหลวงชนบทสายทาง {convertToThaiAlp(data.estimate.route.start_road_code) || 'ชบ.๓๐๐๙'} {convertToThaiAlp(data.estimate.route.start_road) || 'แยกทางหลวงหมายเลข ๓๓๑ - ท่าเรือแหลมฉบัง อำเภอศรีราชา จังหวัดชลบุรี'} ช่วง กม.ที่ {convertToThaiAlp(value.start_km as string) || '๑๘+๗๐๐'} ถึง กม.ที่ {convertToThaiAlp(value.end_km as string) || '๑๙+๐๐๐'} ระยะทางรวม {convertToThaiNumber(value.distance as number) || '๐.๓๐๐'} กิโลเมตร</Text>
        </View>
        <View>
