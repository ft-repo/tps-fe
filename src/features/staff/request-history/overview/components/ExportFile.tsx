            ...
            <View style={styles.table}>
              ...
              {data.data.map((item, index) => (
                <View key={index} style={styles.tableRow}>
                  ...
                  <View style={[styles.tableCell, styles.col7]}>
                    <Text>{item.petition_flow?.[0]?.is_approved ?? false ? 'ผ่าน' : 'ไม่ผ่าน'}</Text>
                  </View>
                  <View style={[styles.tableCell, styles.col8]}>
                    <Text>{item.petition_flow?.[1]?.is_approved ?? false ? 'ผ่าน' : 'ไม่ผ่าน'}</Text>
                  </View>
                  <View style={[styles.tableCell, styles.col9]}>
                    <Text>{item.petition_flow?.[2]?.is_approved ?? false ? 'ผ่าน' : 'ไม่ผ่าน'}</Text>
                  </View>
                  <View style={[styles.tableCell, styles.col10]}>
                    <Text>{item.petition_flow?.[3]?.is_approved ?? false ? 'ผ่าน' : 'ไม่ผ่าน'}</Text>
                  </View>
                  <View style={[styles.tableCell, styles.col11]}>
                    <Text>{item.petition_flow?.[4]?.is_approved ?? false ? 'ผ่าน' : 'ไม่ผ่าน'}</Text>
                  </View>
                </View>
              ))}
            </View>
    ...
    try {
      ...
      data.data.forEach(item => {
        sheet.addRow([
          ...
          item.petition_flow?.[0]?.is_approved ?? false ? 'ผ่านการตรวจ' : 'ไม่ผ่านการตรวจ',
          item.petition_flow?.[1]?.is_approved ?? false ? 'ผ่านการตรวจ' : 'ไม่ผ่านการตรวจ',
          item.petition_flow?.[2]?.is_approved ?? false ? 'ผ่านการตรวจ' : 'ไม่ผ่านการตรวจ',
          item.petition_flow?.[3]?.is_approved ?? false ? 'ผ่านการตรวจ' : 'ไม่ผ่านการตรวจ',
          item.petition_flow?.[4]?.is_approved ?? false ? 'ผ่านการตรวจ' : 'ไม่ผ่านการตรวจ',
        ]);
      });
